...
export default function DisplayPage() {
  ...
  async function load() {
    // ✅ 사실상 전체 랭킹 다 가져오게 limit 크게
    const res = await fetch("/api/display/ranking?limit=999", {
      cache: "no-store",
    });
  ...
  const top2 = rows[1];
  const top3 = rows[2];
  const rest = rows.slice(3); // ✅ 4위부터 꼴찌까지
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-white relative">
      ...
      {/* 화면 전체를 flex-col 로, 아래 리스트가 남은 높이 전부 사용 */}
      <div className="relative h-screen w-screen px-10 py-10 flex flex-col">
        {/* 헤더 */}
        ...
        <div className="mt-8 grid grid-cols-3 gap-6">
          <div className="rounded-3xl border border-emerald-400/25 bg-emerald-400/10 p-7 shadow-[0_0_50px_rgba(34,197,94,0.18)]">
            ...
            <div className="mt-2 text-5xl font-black">
              {top1 ? top1.name || top1.studentId : "—"}
            </div>
            <div className="mt-2 text-xl text-white/75">
              {top1 ? top1.studentId : ""}
            </div>
            <div className="mt-6 text-6xl font-black text-emerald-300">
              {top1 ? nf(top1.points) : "0"}
            </div>
          </div>
          ...
          <div className="rounded-3xl border border-white/10 bg-white/5 p-7 backdrop-blur-xl">
            ...
            <div className="mt-2 text-4xl font-black">
              {top3 ? top3.name || top3.studentId : "—"}
            </div>
            <div className="mt-2 text-lg text-white/70">
              {top3 ? top3.studentId : ""}
            </div>
            <div className="mt-6 text-5xl font-black text-emerald-200">
              {top3 ? nf(top3.points) : "0"}
            </div>
          </div>
        </div>

        {/* 4위 이하 영역: 남은 높이 전부 차지 + 내부 스크롤 */}
        <div className="mt-6 flex-1 min-h-0 rounded-3xl border border-white/10 bg-white/5 backdrop-blur-xl flex flex-col overflow-hidden">
          {/* 헤더 고정 */}
          ...
          <div className="flex-1 min-h-0 overflow-y-auto divide-y divide-white/5">
            {rest.map((r) => (
              <div
                ...
                className="grid grid-cols-[140px_1fr_220px] px-8 py-5 hover:bg-white/5 transition"
              >
                <div className="text-2xl font-black text-white/85">
                  {r.rank}위
                </div>
                <div>
                ...
                <div className="text-right text-3xl font-black text-emerald-200">
                  {nf(r.points)}
                </div>
              </div>
            ))}
